...
/**
 * Überwacht angeschlossene Midi Geräte und ihre Inputs
 */
  ...
  enableMidi() {
    ...
    WebMidi.enable((err) => {
      ...
      if (err) {
        ...
        input.addListener('controlchange', "all", (e) => { // console.log(e);
          switch (e.controller.number) {
            case 0:
              this.server.ChangeSpeed({
                speed: Math.max(0.0001, e.value / 127)
              });
              break;
            case 1:
              var frequ = {
                frequency: (e.value / 127) * 3
              };
